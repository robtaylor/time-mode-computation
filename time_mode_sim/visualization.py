...
class SignalVisualizer:
    ...
    def plot_multiple_signals(
        ...
        fig, axes = plt.subplots(
            n_signals, 1, figsize=(self.figsize[0], self.figsize[1] * n_signals / 3), sharex=True
        )
...
class WaveformAnalyzer:
    ...
    @staticmethod
    def compute_timing_jitter(
        signals: list[TimeSignal], reference_signal: TimeSignal | None = None
    ) -> float:
        """
    ...
    def compute_snr(signal: TimeSignal, noise_signals: list[TimeSignal]) -> float:
        ...
        noise_powers = [
            (s.get_pulse_width() - signal.get_pulse_width()) ** 2 for s in noise_signals
        ]
...
class PerformanceAnalyzer:
    ...
    def compute_throughput(self, signals: list[TimeSignal], phase_duration: float) -> float:
        """
    ...
    def analyze_vmm_performance(
        self, input_size: int, output_size: int, phase_duration: float
    ) -> dict:
        """
...
class NetworkVisualizer:
    ...
    def plot_layer_activations(
        ...
        fig, axes = plt.subplots(
            1, n_layers, figsize=(self.figsize[0] * n_layers / 3, self.figsize[1])
        )

        if n_layers == 1:
            axes = [axes]

        for i, (outputs, name) in enumerate(zip(layer_outputs, layer_names, strict=False)):
    ...
    def plot_training_history(self, history: dict, metrics: list[str] = None) -> plt.Figure:
        ...
        fig, axes = plt.subplots(
            1, n_metrics, figsize=(self.figsize[0] * n_metrics / 2, self.figsize[1] / 2)
        )
        ...
        for phase, color in zip(phases, colors, strict=False):
            ...
            if "signals" in phase:
                for i, signal in enumerate(phase["signals"]):
                    ...
                    times = np.linspace(phase["start"], phase["start"] + phase["duration"], 100)
                    levels = [signal.get_level_at(t - phase["start"]).value * 0.15 for t in times]
