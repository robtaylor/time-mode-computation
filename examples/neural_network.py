...
def example_simple_network():
    ...
    network = TimeNeuralNetwork(
        layer_sizes=layer_sizes, activations=activations, vmm_params={"phase_duration": 1.0}
    )
    ...
    for i, x in enumerate(X):
        ...
        input_signals = [TimeSignal.from_analog_value(val, encoding="pulse_width") for val in x]
...
def example_multilayer_network():
    ...
    print(f"Network architecture: {layer_sizes}")
    print(
        f"Total parameters: {sum(l1 * l2 for l1, l2 in zip(layer_sizes[:-1], layer_sizes[1:], strict=False))}"
    )
    ...
    for i, (layer, ax) in enumerate(zip(network.layers, axes, strict=False)):
    ...
    fig_activations = viz.plot_layer_activations(
        layer_outputs, layer_names=[f"Layer {i + 1}" for i in range(len(layer_outputs))]
    )
...
def example_performance_analysis():
    ...
    print("-" * 40)
    print(f"This work (simulated): ~{np.mean(fj_ops):.1f} fJ/Op")
...
def example_autoencoder():
    ...
    autoencoder = TimeAutoencoder(
        input_size=input_size, encoding_size=encoding_size, hidden_sizes=[4]
    )
